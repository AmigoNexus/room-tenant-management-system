...
const flatSchema = z.object({
  ...
  maintenanceAmount: z.coerce.number().min(0, 'Maintenance must be positive'),
...
interface AddFlatModalProps {
  ...
  floors: Floor[];
  onSuccess: () => void;
  defaultFloorId?: string | number;
  editFlat?: Flat | null;
}

export default function AddFlatModal({ open, onOpenChange, floors, onSuccess, defaultFloorId, editFlat }: AddFlatModalProps) {
  ...
  const form = useForm<FlatFormValues>({
    ...
    defaultValues: {
      ...
      rentAmount: editFlat?.rentAmount || 0,
      depositAmount: editFlat?.depositAmount || 0,
      maintenanceAmount: editFlat?.maintenanceAmount || 0,
      status: editFlat?.status || 'AVAILABLE',
      flatType: editFlat?.flatType || '',
      floorId: editFlat ? String(editFlat.floorId) : (defaultFloorId ? String(defaultFloorId) : (floors.length > 0 ? String(floors[0].id) : '')),
    },
  });
  ...
  React.useEffect(() => {
    if (open) {
      if (editFlat) {
        form.reset({
          flatNumber: editFlat.flatNumber,
          rentAmount: editFlat.rentAmount,
          depositAmount: editFlat.depositAmount,
          maintenanceAmount: editFlat.maintenanceAmount,
          status: editFlat.status,
          flatType: editFlat.flatType || '',
          floorId: String(editFlat.floorId),
        });
      } else {
        form.reset({
          flatNumber: '',
          ...
          status: 'AVAILABLE',
          flatType: '',
          floorId: defaultFloorId ? String(defaultFloorId) : (floors.length > 0 ? String(floors[0].id) : ''),
        });
      }
    }
  }, [open, editFlat, defaultFloorId, floors, form]);

  React.useEffect(() => {
    if (open && !editFlat) {
      if (defaultFloorId) {
        form.setValue('floorId', String(defaultFloorId));
      } else if (floors.length > 0 && !form.getValues('floorId')) {
        form.setValue('floorId', String(floors[0].id));
      }
    }
  }, [open, defaultFloorId, floors, form, editFlat]);

  const onSubmit = async (values: FlatFormValues) => {
    setLoading(true);
    try {
      if (editFlat) {
        await propertyService.updateFlat(editFlat.id, {
        ...
        toast({ title: 'Flat updated', description: 'The flat details have been updated successfully.' });
      } else {
        ...
        toast({ title: 'Flat added', description: 'The new flat was added successfully.' });
      }
      onSuccess();
      form.reset();
      onOpenChange(false);
      ...
      <DialogContent className="sm:max-w-130 rounded-4xl border-none shadow-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="font-medium text-slate-500">
            {editFlat ? 'Update the details of this flat.' : 'Add a flat to an existing floor.'}
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 pt-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='flex flex-col gap-1'>
                    <FormLabel className="text-slate-700 font-bold">Flat Type</FormLabel>
                    <FormControl>
                      <div className="relative">
                        <Input
                          placeholder="e.g. 1BHK, Shop"
                          list="flatTypeOptions"
                          className="h-12 rounded-xl bg-slate-50 border-slate-100 focus:ring-indigo-600 focus:bg-white transition-all uppercase"
                          {...field}
                        />
                        <datalist id="flatTypeOptions">
                          <option value="RK" />
                          <option value="1BHK" />
                          <option value="2BHK" />
                          <option value="3BHK" />
            ...
            <DialogFooter className="pt-4">
              ...
              <Button type="submit" disabled={loading || floors.length === 0} className="rounded-xl font-black h-12 px-8 bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all active:scale-95">
                {loading ? (editFlat ? 'Updating...' : 'Adding...') : (editFlat ? 'Update Flat' : 'Add Flat')}
              </Button>
            </DialogFooter>
          </form>
        </Form>
